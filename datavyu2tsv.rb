...
begin
  ...
  logFile.write("datavyu2tsv initated at: #{Time.now.strftime('Date: %m/%d/%Y Time: %H:%M')}")
      ...
      if columnList.nil?
        ...
        columnList.each do |col_str|
          ...
          if firstCell.nil?
            ...
            # create output file
            newFileStr = "#{col_str}__#{filebasename}.tsv"
            tsv_out = File.new(File.join(outputDir, newFileStr), "wb")
            tsv_out.write("#{['file', 'column', 'ordinal', 'onset', 'offset', args].join("\t")}")
            ...
            # make empty line at end of file and write log info and close tsv file
            tsv_out.write("\n")
            logText = ".....done with .tsv file"
            puts logText
            logFile.write("\n#{logText}\n")
            tsv_out.close
