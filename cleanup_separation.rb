...
def makefile(pathname)
    desktopPointer = File.expand_path("C:\\Users\\acaniglia\\Documents\\Separated\\#{pathname.gsub('.opf', '_cleanup')}.csv")
  ...
  data = []
    ...
    toMyFile = makefile("#{file}")
     ...
     cleanup =  getColumn("cleanup")
        ...
        p_speech.cells.each do | thisp_speechCell |
            cleanup.cells.each do | thiscleanup_Cell |

                if thisp_speechCell.is_within(thiscleanup_Cell)
        ...
        c_speech.cells.each do | thisc_speechCell |
            cleanup.cells.each do | thiscleanup_Cell |
                ...
                if thisc_speechCell.is_within(thiscleanup_Cell)
