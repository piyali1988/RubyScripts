...
def makefile(pathname)
    desktopPointer = File.expand_path("Y:\\New Reorganization\\Research\\TMW\\TMW Longitudinal\\CHILDES material\\Transcription\\Completed Transcripts\\CSV_exports\\#{pathname.gsub('.opf', '_read') + '_read'}.csv")
  ...
  data = []
    ...
    toMyFile = makefile("#{file}")
     ...
     read = getColumn('read')
        ...
        p_speech.cells.each do | thisp_speechCell |
            read.cells.each do | thisread_Cell |
        ...
        c_speech.cells.each do | thisc_speechCell |
            read.cells.each do | thisread_Cell |
                ...
                if thisc_speechCell.is_within(thisread_Cell)
